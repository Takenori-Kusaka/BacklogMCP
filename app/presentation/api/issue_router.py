...
@router.get("/{issue_id_or_key}", response_model=Dict[str, Any], operation_id="get_issue")
async def get_issue(
    issue_id_or_key: str,
    issue_service: IssueService = Depends(get_issue_service)
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get issue {issue_id_or_key}: {str(e)}"
        )


@router.post("/", response_model=Dict[str, Any], status_code=201, operation_id="create_issue")
...
@router.patch("/{issue_id_or_key}", response_model=Dict[str, Any], operation_id="update_issue")
...
@router.post("/{issue_id_or_key}/comments", response_model=Dict[str, Any], status_code=201, operation_id="add_comment")
...
@router.get("/{issue_id_or_key}/comments", response_model=List[Dict[str, Any]], operation_id="get_issue_comments")
