...
async def get_issues_handler(params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return issue_service.get_issues(project_id=project_id, keyword=keyword, count=count)
...
get_issue_tool = Tool(
    ...
    inputSchema={
        "type": "object",
        "properties": {
            "issue_id_or_key": {"type": "string", "description": "課題IDまたは課題キー"}
        },
...
create_issue_tool = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            ...
            "issue_type_id": {"type": "integer", "description": "課題の種別ID"},
            "issue_type_name": {
                "type": "string",
                "description": "課題の種別名（issue_type_idが指定されていない場合に使用）",
            },
            "priority_id": {"type": "integer", "description": "優先度ID"},
            "priority_name": {
                "type": "string",
                "description": "優先度名（priority_idが指定されていない場合に使用）",
            },
            "description": {"type": "string", "description": "課題の詳細"},
...
update_issue_tool = Tool(
    ...
    inputSchema={
        ...
        "properties": {
            ...
            "summary": {"type": "string", "description": "課題の件名"},
            "description": {"type": "string", "description": "課題の詳細"},
            "status_id": {"type": "integer", "description": "ステータスID"},
            "status_name": {
                "type": "string",
...
async def add_comment_handler(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    issue_service = get_issue_service()
    return issue_service.add_comment(issue_id_or_key=issue_id_or_key, content=content)
