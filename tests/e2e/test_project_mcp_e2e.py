...
async def test_mcp_get_projects(mcp_server_url):
    ...
    try:
        ...
        logger.info(f"MCPエンドポイントのContent-Type: {response.headers.get('Content-Type')}")
        # 最初の数バイトだけ読み取る
    ...
    try:
        ...
        logger.info(f"利用可能なツール一覧: {[tool.name for tool in tools_result.tools]}")
        
        # プロジェクト一覧取得
        # FastApiMCP 0.3.3では、FastAPIのエンドポイントのoperation_idがMCPツールの名前として使用される
        # しかし、実際には異なる名前が使用される可能性がある
        # 可能性のある名前を順番に試す
        tool_names = ["get_projects", "api_projects__get", "api/projects/", "api/projects"]
...
async def test_mcp_get_project_by_key(mcp_server_url):
    ...
    try:
        ...
        # しかし、実際には異なる名前が使用される可能性がある
        # 可能性のある名前を順番に試す
        tool_names = ["get_project", "api_projects__project_key__get", "api/projects/{project_key}", "api/projects/{project_key}/"]
        
        # 利用可能なツール一覧から、最も近いツール名を追加
        for tool in tools_result.tools:
            if "project" in tool.name.lower() and "key" in tool.name.lower() and tool.name not in tool_names:
                tool_names.insert(0, tool.name)
        ...
        for tool_name in tool_names:
            try:
                logger.info(f"ツール名試行: {tool_name}")
                result = await session.call_tool(tool_name, {"project_key": project_key})
                logger.info(f"成功したツール名: {tool_name}")
                logger.info(f"結果: {result}")
                break
            except Exception as e:
