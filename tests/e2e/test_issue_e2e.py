...
async def test_get_issues_from_real_api(mcp_server_url):
...
async def test_get_issue_by_key(mcp_server_url):
...
async def test_create_issue_with_name_parameters(mcp_server_url):
...
async def test_update_issue(mcp_server_url):
...
async def test_update_issue_with_name_parameters(mcp_server_url):
    ...
    async with sse_client(mcp_server_url) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            try:
                ...
                assert hasattr(update_result, "content")
                assert update_result.content["issueKey"] == issue_key
                assert update_result.content["summary"] == "名前ベースで更新されたE2Eテスト用課題"
                
                # 更新された課題を取得して確認
                get_result = await session.call_tool("get_issue", {"issue_id_or_key": issue_key})
                assert hasattr(get_result, "content")
                assert get_result.content["issueKey"] == issue_key
                assert get_result.content["summary"] == "名前ベースで更新されたE2Eテスト用課題"
            finally:
                # 課題削除（クリーンアップ）
                delete_result = await session.call_tool("delete_issue", {"issue_id_or_key": issue_key})
                assert hasattr(delete_result, "content")
...
async def test_add_and_get_comments(mcp_server_url):
    ...
    async with sse_client(mcp_server_url) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            try:
                ...
                assert comment_result.content["content"] == "これはE2Eテスト用のコメントです。"
                ...
                assert any(c["content"] == "これはE2Eテスト用のコメントです。" for c in comments_result.content)
            finally:
                # 課題削除（クリーンアップ）
                delete_result = await session.call_tool("delete_issue", {"issue_id_or_key": issue_key})
                assert hasattr(delete_result, "content")
...
async def test_create_and_delete_issue(mcp_server_url):
