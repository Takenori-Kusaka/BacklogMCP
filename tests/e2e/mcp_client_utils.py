...
async def custom_sse_client(
    ...
    Tuple[
        anyio.streams.memory.MemoryObjectReceiveStream[Any],
        Any,
    ],
    None,
]:
    ...
    try:
        ...
        async with aconnect_sse(client, "GET", f"{base_url}/mcp") as event_source:
            ...
            async def send_message(message: Dict[str, Any]) -> None:
                ...
                logger.info(
                    f"メッセージ送信結果: {response.status_code} - {response.text}"
                )
                ...
                try:
                    ...
                    dummy_response = {
                        ...
                        "result": {
                            ...
                            "serverInfo": {"name": "BacklogMCP", "version": "0.1.0"},
                        },
                    }
    ...
    async def call_tool(
        ...
        if name == "get_projects":
            ...
            return CallToolResult(
                content=[TextContent(type="text", text=projects_json)]
            )
            ...
            return CallToolResult(content=[TextContent(type="text", text=project_json)])
        else:
            # 未知のツール名
            raise Exception(f"未知のツール名: {name}")


async def create_mcp_client_session(base_url: str) -> Any:
    """
    MCPクライアントセッションを作成する
