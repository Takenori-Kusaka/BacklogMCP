...
class TestBulkOperationsIntegration:
    ...
    def test_bulk_update_status_integration(self, client):
    ...
    def test_bulk_delete_issues_integration(self, client):
        """複数チケットを一括削除する結合テスト"""
        # APIリクエスト
        response = client.post(
            "/api/bulk/delete",
    ...
    def test_bulk_update_status_with_missing_config(self, client):
        """環境変数が設定されていない場合のエラーハンドリングテスト"""
        # このテストはスキップ - 実際の環境変数を使用するため
        pass
    
    def test_bulk_update_status_with_invalid_request(self, client):
