...
class TestIssueService:
    ...
    def test_get_issues_returns_list(self, mock_backlog_client):
    ...
    def test_get_issues_with_parameters(self, mock_backlog_client):
    ...
    def test_get_issues_with_status_id(self, mock_backlog_client):
        """ステータスIDによるフィルタリングのテスト"""
        # モックの戻り値を設定
        mock_backlog_client.get_issues.return_value = [
            {"id": 1, "issueKey": "TEST-1", "summary": "テスト課題1", "status": {"id": 2, "name": "処理中"}}
        ]
    ...
    def test_get_issues_with_assignee_id(self, mock_backlog_client):
        """担当者IDによるフィルタリングのテスト"""
        # モックの戻り値を設定
        mock_backlog_client.get_issues.return_value = [
            {"id": 1, "issueKey": "TEST-1", "summary": "テスト課題1", "assignee": {"id": 123, "name": "テストユーザー"}}
        ]
    ...
    def test_get_issues_handles_empty_response(self, mock_backlog_client):
        """課題一覧が空の場合の処理をテスト"""
        # モックの戻り値を空リストに設定
        mock_backlog_client.get_issues.return_value = []
    ...
    def test_get_issues_handles_error(self, mock_backlog_client):
        """エラー発生時の処理をテスト"""
        # モックがエラーを発生させるように設定
    ...
    def test_get_issue_returns_dict(self, mock_backlog_client):
    ...
    def test_get_issue_handles_not_found(self, mock_backlog_client):
        """存在しない課題の場合の処理をテスト"""
        # モックの戻り値をNoneに設定
    ...
    def test_get_issue_handles_error(self, mock_backlog_client):
        """エラー発生時の処理をテスト"""
        # モックがエラーを発生させるように設定
    ...
    def test_create_issue_returns_dict(self, mock_backlog_client):
    ...
    def test_create_issue_with_name_parameters(self, mock_backlog_client):
        """名前ベースのパラメータで課題を作成するテスト"""
        # モックの戻り値を設定
        mock_backlog_client.create_issue.return_value = {
            "id": 1, "issueKey": "TEST-1", "summary": "名前ベースの課題"
        }
    ...
    def test_create_issue_handles_error(self, mock_backlog_client):
        """エラー発生時の処理をテスト"""
        # モックがエラーを発生させるように設定
    ...
    def test_update_issue_returns_dict(self, mock_backlog_client):
    ...
    def test_update_issue_with_name_parameters(self, mock_backlog_client):
        """名前ベースのパラメータで課題を更新するテスト"""
        # モックの戻り値を設定
        mock_backlog_client.update_issue.return_value = {
            "id": 1, "issueKey": "TEST-1", "summary": "名前ベースで更新された課題",
            "status": {"id": 2, "name": "処理中"},
            "assignee": {"id": 1, "name": "テストユーザー1"}
        }
        mock_backlog_client.get_issue.return_value = {
            "id": 1, "issueKey": "TEST-1", "projectId": 1
        }
    ...
    def test_update_issue_with_status_id(self, mock_backlog_client):
        """ステータスIDを指定して課題を更新するテスト"""
        # モックの戻り値を設定
        mock_backlog_client.update_issue.return_value = {
            "id": 1, "issueKey": "TEST-1", "summary": "テスト課題1", "status": {"id": 3, "name": "処理済み"}
        }
    ...
    def test_update_issue_with_assignee_id(self, mock_backlog_client):
        """担当者IDを指定して課題を更新するテスト"""
        # モックの戻り値を設定
        mock_backlog_client.update_issue.return_value = {
            "id": 1, "issueKey": "TEST-1", "summary": "テスト課題1", "assignee": {"id": 456, "name": "新担当者"}
        }
    ...
    def test_update_issue_with_dates(self, mock_backlog_client):
        """開始日・期限日を指定して課題を更新するテスト"""
        # モックの戻り値を設定
        mock_backlog_client.update_issue.return_value = {
            "id": 1, "issueKey": "TEST-1", "summary": "テスト課題1", "startDate": "2025-05-01", "dueDate": "2025-05-31"
        }
    ...
    def test_update_issue_handles_error(self, mock_backlog_client):
        """エラー発生時の処理をテスト"""
        # モックがエラーを発生させるように設定
    ...
    def test_delete_issue_returns_true(self, mock_backlog_client):
    ...
    def test_delete_issue_handles_error(self, mock_backlog_client):
        """エラー発生時の処理をテスト"""
        # モックがエラーを発生させるように設定
    ...
    def test_add_comment_returns_dict(self, mock_backlog_client):
    ...
    def test_add_comment_handles_error(self, mock_backlog_client):
        """エラー発生時の処理をテスト"""
        # モックがエラーを発生させるように設定
    ...
    def test_get_issue_comments_returns_list(self, mock_backlog_client):
    ...
    def test_get_issue_comments_handles_error(self, mock_backlog_client):
        """エラー発生時の処理をテスト"""
        # モックがエラーを発生させるように設定
